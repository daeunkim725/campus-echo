...
export default function Onboarding() {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      const result = await apiVerifyCode(code.trim(), verificationId);
  ...
  const handleMoodSelect = async () => {
    ...
    setLoading(true);
    // For mood, we still need to update the user — this can be done via a simple API call
    // For now we'll proceed and rely on the backend storing mood later
    setLoading(false);
    setStep("done");
    setTimeout(() => {
        ...
        <div className="w-full max-w-xl">
          ...
          {step === "mood" && (
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {MOODS.map(m => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${selectedMood === m.value
              ...
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><ArrowRight className="w-4 h-4" /> Enter Campus Echo</>}
              </button>
            </div>
          )}
