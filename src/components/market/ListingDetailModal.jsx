        ...
        <div className="bg-white w-full h-[90vh] sm:h-auto sm:max-h-[90vh] sm:max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-8 sm:zoom-in-95" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="p-5 space-y-5 relative">
              {listing.status === 'sold' && (
                <div className="absolute top-0 left-0 right-0 bg-slate-800 text-white py-2 px-5 text-sm font-bold shadow-md flex items-center justify-center gap-2">
                  <CheckCircle2 className="w-4 h-4" /> This item has been sold
                </div>
              )}
              <div className={listing.status === 'sold' ? 'pt-6' : ''}>
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-medium pb-4 border-b border-slate-50 mb-4">
            ...
            <div className="p-4 border-t border-slate-100 shrink-0 bg-white shadow-[0_-4px_15px_-5px_rgba(0,0,0,0.05)]">
              {isSeller ? (
                ...
                <button
                  ...
                  className={`w-full py-3.5 font-bold rounded-xl text-[15px] flex items-center justify-center gap-2 transition-colors ${listing.status === 'sold' ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'text-white hover:opacity-90'}`}
                  style={listing.status !== 'sold' ? { backgroundColor: tokens.primary, color: tokens.surface } : {}}
                >
